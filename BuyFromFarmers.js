...
function renderVegetables() {
    ...
    vegetablesData.forEach((vegetable) => {
        ...
        vegetableCard.innerHTML = `
            ...
            <span>
            <button class="- cnt" onclick="decrease(${vegetable.id},100)">-100</button>
            <button class="- cnt" onclick="decrease(${vegetable.id},10)">-10</button>
            <button class="- cnt" onclick="decrease(${vegetable.id},1)">-1</button>
            <button class="+ cnt" onclick="increase(${vegetable.id},1)">+1</button>
            <button class="+ cnt" onclick="increase(${vegetable.id},10)">+10</button>
            <button class="+ cnt" onclick="increase(${vegetable.id},100)">+100</button>
            </span>
            <p><span class="count">1</span>kg</p>
            <p>Price: <span class="price">$${vegetable.price.toFixed(2)}</span></p>
            <button class="submit" onclick="addToCart(${vegetable.id})">Add to Cart</button>
        `;
...
function addToCart(vegetableId) {
    ...
    if(selectedVegetable in cart)
    cart.pop(selectedVegetable);
    cart.push(selectedVegetable);
    updateCart();
}

function increase(vegId,n){
    var count=document.getElementsByClassName("count")[parseInt(vegId)-1];
    var price=document.getElementsByClassName("price")[parseInt(vegId)-1];
    var priced=parseInt(count.innerHTML)+n;
    count.innerHTML=priced;
    price.innerHTML=(vegetablesData[parseInt(vegId)-1].price)*priced;
}

function decrease(vegId,n){
    var price=document.getElementsByClassName("price")[parseInt(vegId)-1];
    var count=document.getElementsByClassName("count")[parseInt(vegId)-1];
    var priced=parseInt(count.innerHTML)-n;
    if(priced>=0){
